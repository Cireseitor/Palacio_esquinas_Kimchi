...
public class Demo : MonoBehaviour {
    ...
    void OnGUI() {
        ...
        // Post Tweet Button
        rect.x = Screen.width * POST_TWEET_X;
        rect.y = Screen.height * POST_TWEET_Y + 50;
        rect.width = Screen.width * POST_TWEET_WIDTH;
        rect.height = Screen.height * POST_TWEET_HEIGHT;

        if (GUI.Button(rect, "Post Image")) {
            var image = File.ReadAllBytes(Application.persistentDataPath + "/fotos/KinectPhoto_20170607_134609.jpg");
            StartCoroutine(Twitter.API.PostImage(image, CONSUMER_KEY, CONSUMER_SECRET, m_AccessTokenResponse,
                          new Twitter.PostImageCallback(this.OnPostImage)));
        }
    ...
    void OnPostImage(bool success, Twitter.PostImageResponse response) {
        if (success) {
            //post tweet con media id de la respuesta
        }
        else {
